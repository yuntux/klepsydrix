...
**`info={"label": None}` — champ sans libellé, dont le widget récupère la place (fiche uniquement)** : un libellé connaît **trois** états, et `None` n'est ni « clé absente » ni `""`. Clé absente : aucun libellé déclaré, repli sur le nom du champ. Chaîne, **même vide** : libellé affiché tel quel — `""` réserve sa colonne sans rien y écrire, ce qui reste utile pour aligner un champ sous les autres. `None` : aucun libellé, et le widget de saisie occupe **en plus** la colonne du label (`span 2` au lieu de `auto`, `span 4` au lieu de `span 3` pour un champ `fullWidth`). C'est un cumul de largeur, pas un masquage : rien ne doit rester en blanc à gauche du champ.

La règle ne vaut **que pour la fiche** (`GenericForm.vue`) : une colonne de liste sans en-tête n'aurait pas de sens, donc `App.vue::buildColumnsConfig` continue de retomber sur le nom du champ. D'où un drapeau `labelHidden` porté séparément sur le champ de formulaire, plutôt qu'un `label` mis à `null` dans le tableau `fields` partagé par les deux vues.

Le libellé se surcharge aussi côté `ui.json` : `overrideLabel` sur un élément de layout (`label` y est accepté en second recours), et `label` sur un champ d'étape de wizard. `null` y a le même sens que sur le modèle. Attention au piège que cela crée dans le code : `elem.overrideLabel || original.label` retombe sur le libellé du modèle, `null` étant *falsy*, ce qui annulait silencieusement la surcharge. La résolution passe donc par une comparaison à `undefined` (`resolveLabel`, `GenericForm.vue`), seule capable de distinguer « pas de surcharge » de « surcharge à null ».

Côté backend, ce troisième état ne peut pas transiter par le `title` de Pydantic : un `title=None` est purement et simplement omis du schéma, donc indiscernable d'un libellé non déclaré. `generic.py::_apply_label` le porte donc dans le schéma étendu, où il se sérialise en `"label": null`, et le frontend lit cette clé.

Contrepartie assumée : l'astérisque de champ requis, l'infobulle d'aide (`info={"help": ...}`) et le badge « Modifié » du multi-édition vivent **dans** le label et disparaissent avec lui. Ce choix ne convient donc qu'aux champs qui se passent des trois — typiquement un encart HTML, un widget auto-porteur, ou un champ dont le sens est évident dans son contexte.
...
- **Par défaut** (`type === 'binary'` sans `widget` déclaré, `BinaryFileField.vue`) : bouton **Parcourir…**, nom de fichier / état, puis boutons Télécharger / Effacer (upload via `<input type="file">` + `FileReader`, remplace la valeur existante). L'ordre n'est pas arbitraire : « Parcourir… » est la seule action toujours disponible, et la seule qui ait un sens sur un champ vide — elle est donc placée en tête et nommée en toutes lettres, là où une icône seule se faisait rejeter à l'extrême droite par le `flex: 1` du nom de fichier. Télécharger reste offert en lecture seule (consulter n'est pas modifier), Parcourir et Effacer non. Câblé comme branche native de `FormLayoutGrid` (au même niveau que `text`/`date`/`color`...), pas via le registre de widgets — c'est le comportement de **tout** champ binaire qui ne demande rien de plus.
...
**Une étape accepte les nœuds de layout, pas seulement des champs.** `{type: "group", string, children}`, `separator`, `newline`, `notebook`/`page` se mêlent aux champs d'une étape exactement comme dans un `formConfig.fields` classique. Deux pièges se referment là-dessus, et tous deux échouaient *en silence* :

- **`type` porte deux vocabulaires.** Sur un nœud de layout c'est sa nature (`group`…), sur un champ c'est son widget (`boolean`, `html`…). `parseLayoutElement` (`GenericForm.vue`) reconnaissait une référence de champ par `elem.key && !elem.type` : un `{key, label, type: "boolean"}` posé dans un groupe était donc rejeté, et disparaissait sans message. Le test porte désormais sur l'appartenance à `LAYOUT_NODE_TYPES`.
- **GenericForm attend DEUX listes, une étape n'en déclare qu'une.** `fields` est la liste plate où il résout chaque `key` ; `formConfig.fields` est l'arbre de disposition. `GenericWizard.vue` dédouble donc la liste de l'étape (`flattenStepFields` + `stepLayout`) — sans quoi un groupe s'affiche, mais vide. `stepLayout` vaut `undefined` quand l'étape ne déclare aucun nœud de structure : les étapes existantes retombent alors sur le repli habituel, tous les champs à plat, comportement strictement inchangé.

**La clé d'un champ de saisie doit exister dans `_fields`.** Un champ d'affichage (`html`, tableau d'aperçu) peut légitimement n'être alimenté que par le résultat d'un RPC fusionné dans le brouillon ; un champ de saisie, lui, tire sa valeur INITIALE du modèle — absent de `_fields`, il n'est ni sérialisé par `read()` ni déclaré au schéma, donc la case part décochée quel que soit le défaut Python, sans le moindre avertissement. `test_generic.py::TestChampsDesWizards` verrouille la règle pour tous les wizards, groupes traversés.
