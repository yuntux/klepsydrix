...
async def stop_solver_if_running(page, timeout=2000, wait_gone_timeout=25000, wait_backend_timeout=15000):
    ...
    Piège plus sournois, trouvé en creusant pourquoi les étapes 9-10 échouaient de façon
    reproductible (4 runs identiques) : même une fois l'overlay disparu CÔTÉ CLIENT, le backend
    peut mettre un peu plus de temps à repasser son état interne (SolverState) à "NOT_SOLVING".
    Tant que ce n'est pas fait, `start_course_placement_async`/`start_classroom_assignment_async`/
    `start_optimize_pipeline_async` (backend/app/solver/solver.py) sont des NO-OP SILENCIEUX :
    `if SolverState.get_status(slug) in ("SOLVING", "QUEUED"): return` — mais le RPC appelant
    répond quand même {"status": "success"} sans jamais avoir rien démarré. Rien ne le signale
    côté IHM : le wizard suivant reste simplement ouvert, aucun overlay solveur n'apparaît jamais.
    D'où le sondage explicite de GET /api/timetable/status ci-dessous, jusqu'à lire "NOT_SOLVING"
    côté backend — la disparition de l'overlay ne suffit pas à le garantir."""
    try:
    ...
    try:
        deadline = time.time() + wait_backend_timeout / 1000
        while time.time() < deadline:
            resp = await page.request.get(
                f"{BASE_URL}/api/timetable/status",
                headers={"X-Klepsydrix-Database": DEMO_DB},
            )
            if resp.ok:
                data = await resp.json()
                if data.get("status") == "NOT_SOLVING":
                    return
...
async def submit_wizard_step(page, timeout=8000):
    ...
    Piège découvert en creusant l'échec reproductible de l'étape 9 (Optimiser) : `.count()` est
    une lecture INSTANTANÉE du DOM, pas une attente — contrairement à `.click()`, qui attend
    l'actionabilité. Le wizard charge ses valeurs par défaut de façon asynchrone
    (WizardOptimizeTimetable.read()) avant de rendre le formulaire ; sur cette étape précise, plus
    tardive dans la démo (session déjà bien chargée en interactions), ce rendu met parfois plus de
    temps que le tour d'horloge suivant .wait_for_selector(".generic-wizard") — le `count()`
    tombait alors à 0 et la fonction abandonnait aussitôt sans jamais retenter, alors que le
    bouton apparaissait une fraction de seconde plus tard. `wait_for(state="visible")` attend
    réellement l'apparition au lieu d'un simple instantané."""
    try:
