<template>
  <div
    ...
    :style="{
      backgroundColor: resolvedColor || 'var(--bg-card)',
      height: height || 'auto',
      bottom: isPlaced ? 'auto' : undefined,
      zIndex: isPlaced ? 10 : undefined,
      position: isPlaced ? 'absolute' : 'relative',
      color: resolvedColor ? '#0F172A' : 'var(--text-primary)',
      borderLeftColor: resolvedColor ? 'rgba(0, 0, 0, 0.1)' : 'var(--accent-primary)',
  ...
  >
    <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 4px;">
      <span class="placed-subject">{{ resolvedSubjectLabel }}</span>
      <div v-if="isPlaced" style="display: flex; align-items: center; gap: 2px;">
        <button @click.stop="$emit('togglePin', course.id)" class="pin-btn" :class="{ 'is-pinned': course.is_pinned }" :title="course.is_pinned ? 'Déverrouiller le cours' : 'Verrouiller le cours'" :style="{ color: resolvedColor ? '#334155' : undefined }">
        ...
        <button @click.stop="$emit('unassign', course.id)" class="unassign-btn" title="Retirer de la grille" :style="{ color: resolvedColor ? '#334155' : undefined }">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-end; flex: 1; min-height: 0; gap: 4px; overflow: hidden;">
      <div class="placed-meta" :style="{ color: resolvedColor ? '#334155' : 'var(--text-secondary)' }">
...
// subject_id est la seule source de vérité (voir architecture.md §15.U) : subject/color n'est
// plus fourni par le backend (GET /api/timetable a disparu), on retrouve la matière dans le
// cache déjà partagé côté grille (subjectsList, transmis en cascade depuis App.vue). Fallbacks
// repris tels quels des usages existants avant cette conversion (Sidebar.vue/TimetableGrid.vue
// pour la couleur, CoursePopin.vue pour le libellé).
const resolvedSubject = computed(() => props.subjects?.find((s: any) => s.id === props.course.subject_id));
const resolvedColor = computed(() => resolvedSubject.value?.color || '#cbd5e1');
const resolvedSubjectLabel = computed(() => resolvedSubject.value?.short_name || 'Aucune Matière');
</script>
