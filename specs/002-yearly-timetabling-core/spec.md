...
- **Sélection obligatoire de période** : Lorsqu'un type de période est sélectionné (par exemple Trimestre), l'application sélectionne par défaut toutes les périodes de ce type. Pour assurer la cohérence logique des vœux, l'utilisateur ne peut pas décocher toutes les périodes : la dernière période cochée restante est verrouillée/désactivée à la modification tant qu'une autre période n'est pas cochée.
...
4. **Given** un type de période actif, **When** l'utilisateur essaie de décocher toutes les périodes, **Then** la dernière période restante est désactivée (disabled) pour forcer au moins une période sélectionnée.
